...
def predict(text: str, model, tokenizer):
    ...
    # Remove key if present to prevent architecture mismatches with DistilBERT
    ...
    predicted_probability = probabilities[0][argmax_idx].item()
    
    predicted_label_name = model.config.id2label[argmax_idx]
    
    return predicted_label_name, float(predicted_probability)
...
def apply_to_corpus(csv_path: str, model_hub_id: str, output_path: str) -> None:
    ...
    for _, row in df.iterrows():
        ...
        pred_label, pred_prob = predict(text_content, model, tokenizer)
